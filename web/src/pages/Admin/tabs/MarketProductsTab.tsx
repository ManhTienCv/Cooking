  ...
  const load = async () => {
    ...
    try {
      const d = await apiJson<{ products: AdminProduct[]; total: number }>(
        `/api/admin/marketplace/products?status=${status}&limit=50`
      );
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                {(() => {
                  ...
                  else if (sortBy === 'price_desc') filtered.sort((a,b) => (b.sale_price??b.price) - (a.sale_price??a.price));
                  else if (sortBy === 'stock_low') filtered.sort((a,b) => a.stock - b.stock);
                  if (filtered.length === 0) return <tr><td colSpan={7} className="p-8 text-center text-slate-400">Không tìm thấy sản phẩm phù hợp</td></tr>;
