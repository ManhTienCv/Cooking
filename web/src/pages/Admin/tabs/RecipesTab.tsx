...
export default function RecipesTab() {
  ...
  const onRecipeAction = useCallback(async (id: string, action: string) => {
    if (action === 'delete') {
  ...
  const actions = useCallback((row: Record<string, unknown>) => (
    <div className="flex justify-end gap-3 text-sm font-medium">
      {row.status === 'pending' && (
        <>
          <button onClick={() => void onRecipeAction(String(row.id), 'approve')} className="text-green-600 hover:text-green-800">
            Duyệt
          </button>
          <button onClick={() => void onRecipeAction(String(row.id), 'reject')} className="text-yellow-600 hover:text-yellow-800">
            Từ chối
          </button>
        </>
      )}
      <button onClick={() => void onRecipeAction(String(row.id), 'delete')} className="text-red-600 hover:text-red-800">
        Xóa
      </button>
    </div>
  ), [onRecipeAction]);
